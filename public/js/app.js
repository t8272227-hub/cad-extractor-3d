...
if(pdfFrame&&Number.isFinite(pdfFrame.x1)){var _pf1=cadToScreen(pdfFrame.x1,pdfFrame.y1),_pf2=cadToScreen(pdfFrame.x2,pdfFrame.y2);var _pfx=Math.min(_pf1.x,_pf2.x),_pfy=Math.min(_pf1.y,_pf2.y);var _pfw=Math.abs(_pf2.x-_pf1.x),_pfh=Math.abs(_pf2.y-_pf1.y);cx.strokeStyle=pdfFrameDrawing?'#2563eb':'rgba(37,99,235,0.5)';cx.lineWidth=pdfFrameDrawing?1.5*pr:0.8*pr;cx.setLineDash(pdfFrameDrawing?[]:([6*pr,3*pr]));cx.strokeRect(_pfx,_pfy,_pfw,_pfh);cx.setLineDash([]);cx.fillStyle='rgba(37,99,235,0.05)';cx.fillRect(_pfx,_pfy,_pfw,_pfh);cx.fillStyle='#2563eb';cx.font=(8*pr)+'px sans-serif';cx.fillText('📄 PDF',_pfx+3*pr,_pfy+10*pr);};
}
...
var _sbco=document.getElementById('sb-coords');if(_sbco&&dxfData){var _sc=screenToCad(mx,my);_sbco.textContent='X: '+_sc.x.toFixed(3)+'  Y: '+_sc.y.toFixed(3);}var _sbz=document.getElementById('sb-zoom');if(_sbz)_sbz.textContent=(scale/baseScale).toFixed(1)+'×';if(isDrawingArea){const c=screenToCad(mx,my);exportArea.x2=c.x;exportArea.y2=c.y;requestDraw();}else if(dxfData&&(currentTool==='point'||currentTool==='dimension'||currentTool==='interpolate')){const cm=screenToCad(mx,my),st=15/scale,sts=st*st;
...
function _buildAndSavePDF(pdfMeta,canvasId,pts,dims,lines){
  ...
  var fontPt=5.5; // summary/table → separate DOCX
  var tblHtml='';
...
function sbUpdateSnap(){var n=document.getElementById('sb-snap-nodes'),l=document.getElementById('sb-snap-lines'),m=document.getElementById('sb-snap-mid');var sn=document.getElementById('snap-nodes'),sl=document.getElementById('snap-lines'),sm=document.getElementById('snap-midpoints');if(sn&&n)sn.checked=n.checked;if(sl&&l)sl.checked=l.checked;if(sm&&m)sm.checked=m.checked;updateSnapModes();}
function updateSnapModes(){var n=document.getElementById('sb-snap-nodes'),l=document.getElementById('sb-snap-lines'),m=document.getElementById('sb-snap-mid');var sn=document.getElementById('snap-nodes'),sl=document.getElementById('snap-lines'),sm=document.getElementById('snap-midpoints');if(n&&sn)n.checked=sn.checked;if(l&&sl)l.checked=sl.checked;if(m&&sm)m.checked=sm.checked;

...
function setTool(t){
  ...
  var hints={point:'📍 ЛКМ по узлу DXF',interpolate:'🏔 Интерполяция Z',
    dimension:'📏 Размерная линия',area:'✂ Область PDF'};
...
r.onload=function(ev){
  try{
    ...
    var _detail = 'Не удалось открыть DXF.\n' + err.message;
    var _stack = err.stack || '';
    // Extract first meaningful line from stack
    var _stackLines = _stack.split('\n').filter(function(l){
      return l.includes('app.js') || l.includes('processCADData') || l.includes('DxfParser');
    });
    if(_stackLines.length){
      _detail += '\n\nСтек вызовов:\n' + _stackLines.slice(0,3).join('\n');
    }
    console.error('[DXF ERROR]', err.message, err.stack);
    showMessage('Ошибка', _detail, 'error');
    // Log to diagnostics if available
    if(typeof _DIAG !== 'undefined' && _DIAG.results){
      _DIAG.results.push({label:'DXF load: '+err.message, ok:false});
    }
  }finally{rI();}
};
...
function toggleOrthoMode(){
  ...
  if(btn){btn.style.background=on?'rgba(37,99,235,.3)':'transparent';
    btn.style.borderColor=on?'#2563eb':'transparent';btn.style.color=on?'#93c5fd':'#94a3b8';}
  var sb=document.getElementById('sb-ortho');if(sb)sb.style.display=on?'inline':'none';
  requestDraw();
  showMessage('Ortho',on?'ORTHO включён':'ORTHO выключен','info');
}
function _northFromStatusBar(v){
  northAngle=parseFloat(v)||0;
  var d=document.getElementById('nw-deg');if(d)d.value=northAngle;
  var r=document.getElementById('nw-range');if(r)r.value=northAngle;
  _northDraw&&_northDraw();requestDraw();
}
function _toggleStatusBar(){
  var sb=document.getElementById('bottom-status-bar');if(!sb)return;
  var r2=document.getElementById('sb-row2');
  var btn=sb.querySelector('[data-toggle-sb]');
  var exp=sb.classList.contains('sb-exp');
  sb.classList.toggle('sb-exp');
  if(r2)r2.style.display=exp?'none':'flex';
  if(btn)btn.textContent=exp?'▲':'▼';
}


function _drawNorthOnCanvas(ctx,cw,ch){
  var ang=northAngle*Math.PI/180,cx2=cw-44,cy2=44,r=22;
  ctx.save();ctx.translate(cx2,cy2);
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  ctx.strokeStyle='rgba(30,41,59,0.2)';ctx.lineWidth=1;ctx.stroke();
  ctx.rotate(ang);
  ctx.beginPath();ctx.moveTo(0,-r+2);ctx.lineTo(4,2);ctx.lineTo(0,4);ctx.lineTo(-4,2);ctx.closePath();
  ctx.fillStyle='rgba(15,23,42,0.85)';ctx.fill();
  ctx.beginPath();ctx.moveTo(0,r-2);ctx.lineTo(4,-2);ctx.lineTo(0,-4);ctx.lineTo(-4,-2);ctx.closePath();
  ctx.fillStyle='rgba(255,255,255,0.75)';ctx.strokeStyle='rgba(30,41,59,0.3)';ctx.lineWidth=0.5;ctx.fill();ctx.stroke();
  ctx.beginPath();ctx.arc(0,0,2,0,Math.PI*2);ctx.fillStyle='rgba(30,41,59,0.7)';ctx.fill();
  ctx.rotate(-ang);
  var lx=Math.sin(ang)*14,ly=-Math.cos(ang)*14;
